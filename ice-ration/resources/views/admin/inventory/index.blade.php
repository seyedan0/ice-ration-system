<x-layouts.admin title="Inventory Management">
        ...
        @foreach ($stations as $station)
            <div class="bg-white rounded-xl shadow p-5">
                ...
                <form method="POST" action="{{ route('admin.inventory.adjust', $station) }}" class="flex gap-2">
                    @csrf
                    <input type="number" name="blocks_delta" placeholder="+50 or -20" required
                        class="flex-1 rounded-lg border border-slate-300 px-3 py-2 text-sm">
                    <input type="text" name="reason" placeholder="Reason (optional)"
                        class="flex-1 rounded-lg border border-slate-300 px-3 py-2 text-sm">
                    <button class="px-4 py-2 rounded-lg bg-slate-800 text-white text-sm font-semibold hover:bg-slate-900">Adjust</button>
                </form>
            </div>
        @endforeach
    </div>
    ...
    <div class="bg-white rounded-xl shadow overflow-hidden">
        <div class="px-5 py-4 border-b border-slate-100">
            <h2 class="font-semibold text-slate-800">Recent Inventory Activity</h2>
        </div>
        <table class="w-full text-sm">
            <thead class="bg-slate-50 text-slate-500 text-left">
                <tr>
                    ...
                    <th class="px-5 py-2">Change</th>
                    <th class="px-5 py-2">Stock After</th>
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse ($recentLogs as $log)
                    <tr>
                        ...
                        <td class="px-5 py-2.5 font-semibold {{ $log->blocks_delta >= 0 ? 'text-green-600' : 'text-red-600' }}">
                            {{ $log->blocks_delta >= 0 ? '+' : '' }}{{ $log->blocks_delta }}
                        </td>
                ...
                @empty
                    <tr><td colspan="6" class="px-5 py-6 text-center text-slate-400">No activity yet.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
