<x-layouts.admin :title="$station->exists ? 'Edit Station' : 'New Station'">
    <div class="bg-white rounded-xl shadow p-6 max-w-xl">
        <form method="POST" action="{{ $station->exists ? route('admin.stations.update', $station) : route('admin.stations.store') }}" class="space-y-4">
            ...
            <div>
                <label class="block text-sm font-medium text-slate-700 mb-1">Current Stock (ice blocks)</label>
            ...
            <div class="flex gap-2 pt-2">
                ...
                <a href="{{ route('admin.stations.index') }}" class="px-4 py-2 rounded-lg bg-slate-100 text-sm">Cancel</a>
            </div>
        </form>
    </div>
</x-layouts.admin>
